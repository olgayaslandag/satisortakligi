...
<div class="container">
  <div class="row mt-4 mb-4">
    <div class="col-12 col-sm">
      <div class="btn-group" role="group" aria-label="Basic example">
        ...
        <a class="btn btn-sm btn-secondary" href="#">Cirolar</a>
        ...
        <div class="card card-body mt-3">
          <form @submit.prevent="ekle()">
            <div class="form-group mb-2">
              ...
              <select class="form-select" v-model="form.ciro_yil" required>
                <option :value="2022">2022</option>
                <option :value="2021">2021</option>
              </select>
            </div>
            ...
            <div class="form-group mb-2">
              ...
              <select class="form-select" v-model="form.ciro_ay" required>
                <option v-for="ay in aylar" :key="ay" :value="ay">{{aylarText[ay]}}</option>
              </select>
            </div>
            ...
            <div class="form-group mb-2">
              ...
              <select class="form-select" v-model="form.ciro_seviye" required>
            ...
            <div class="form-group mb-2">
              <label for="yuzde">Min Miktar Girin</label>
              <input type="number" class="form-control" v-model="form.ciro_min" required>
            </div>

            <div class="form-group mb-2">
              <label for="yuzde">Max Miktar Girin</label>
              <input type="number" class="form-control" v-model="form.ciro_max" required>
            </div>

            <div class="form-group">
              <label for="yuzde">Yüzde Girin</label>
              <input type="number" class="form-control" v-model="form.ciro_prim" required>
            </div>
  ...
  <div class="row">
    <div class="col">
      <div class="table-responsive" v-if="cirolar.length > 0">
        <table class="table">
          <thead>
            <tr>
              <th scope="col">Yıl</th>
          ...
          <tbody>
            <tr v-for="ciro in cirolar" :key="ciro.ciro_id">
              <td>{{ciro.ciro_yil}}</td>
              <td>{{ciro.ciro_ay}}</td>
              <td>{{ciro.ciro_seviye}}</td>
              <td>{{ciro.ciro_min}}</td>
              <td>{{ciro.ciro_max}}</td>
              <td>+%{{ciro.ciro_prim}}</td>
              <td width="130">
                <ul class="list-inline m-0">
                  <li class="list-inline-item">
                    <a class="btn btn-secondary badge bg-secondary" href="#" @click="secilenItem = ciro" data-bs-toggle="modal" data-bs-target="#exampleModal">düzenle</a>
                  </li>

                  <li class="list-inline-item">
                    <a class="btn btn-danger badge bg-danger" href="#" @click="sil(ciro.ciro_id)">sil</a>
                  </li>
                </ul>
              </td>
            </tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body" v-if="secilenItem">
        <div class="form-group mb-2">
          <label for="yil">Yıl Seçin</label>
          <select class="form-select" v-model="secilenItem.ciro_yil">
            <option :value="2022">2022</option>
            <option :value="2021">2021</option>
          </select>
        </div>
        ...
        <div class="form-group mb-2">
          <label for="ay">Ay Seçin</label>
          <select class="form-select" v-model="secilenItem.ciro_ay">
            <option v-for="ay in aylar" :key="ay" :value="ay">{{aylarText[ay]}}</option>
          </select>
        </div>
        ...
        <div class="form-group mb-2">
          <label for="seviye">Seviye Seçin</label>
          <select class="form-select" v-model="secilenItem.ciro_seviye">
            <option :value="0">0. Seviye</option>
            <option :value="1">1. Seviye</option>
            <option :value="2">2. Seviye</option>
            <option :value="3">3. Seviye</option>
        ...
        <div class="form-group">
          <label for="yuzde">Min Miktar Girin</label>
          <input type="number" class="form-control" v-model="secilenItem.ciro_min">
        </div>

        <div class="form-group">
          <label for="yuzde">Max Miktar Girin</label>
          <input type="number" class="form-control" v-model="secilenItem.ciro_max">
        </div>

        <div class="form-group">
          <label for="yuzde">Yüzde Girin</label>
          <input type="number" class="form-control" v-model="secilenItem.ciro_prim">
        </div>
      </div>
...
var app = new Vue({
  ...
  data: {
    ...
    cirolar: <?php echo json_encode($cirolar); ?>,
    secilenItem: null,
    ...
    form: {
      ciro_yil: Number(<?php echo json_encode($yil); ?>),
      ciro_ay: Number(<?php echo json_encode($ay); ?>),
      ciro_seviye: <?php echo json_encode($seviye); ?>,
      ciro_min: null,
      ciro_max: null,
      ciro_prim: null,
    },
  ...
  methods: {
    update(){
      axios.post(this.url + "Post_Ayarlar/updateCiro", JSON.stringify(this.secilenItem)).then(r => {
        if(r.data.status){
          var index = this.cirolar.findIndex(c => c.ciro_id == r.data.result.ciro_id);
          if(index > -1){
            this.cirolar[index] = r.data.result;
          }
        }
    ...
    filter(){
      var url = this.url + "Ayarlar/cirolar/";
    ...
    ekle(){
      ...
      axios.post(this.url + "Post_Ayarlar/addCiro", JSON.stringify(this.form)).then(r => {
        if(r.data.status){
          this.cirolar.push(r.data.result);
    ...
    sil(id){
      Swal.fire({
        ...
        if(result.isConfirmed){
          axios.post(this.url + "Post_Ayarlar/deleteCiro", JSON.stringify({ciro_id: id})).then(r => {
            if(r.data.status){
              var index = this.cirolar.findIndex(c => c.ciro_id == id);
              if(index > -1){
                this.cirolar.splice(index, 1);
              }
            }
            Swal.fire({
              icon: r.data.status ? 'success' : 'error',
