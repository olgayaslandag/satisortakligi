...
<?php $this->load->view('header', ['title' => 'Dashboard']); ?>
		<div v-if="user">
	        	...
	        	<form @submit.prevent="guncelle()" class="float-start m-0">
                      ...
                      <option :value="1">Pasif Gizle</option>
						...
						<div class="float-end d-none d-sm-block">
							<button class="btn btn-success" @click="exportTableToExcel('veriler')" style="margin-top: 5px;margin-left: 5px;">
          ...
          <table class="table table-bordered" id="veriler">
						<table class="table table-bordered" id="veriler">
				    ...
				    <div class="modal-content">
				      <div class="modal-body">
			      	    ...
			      	    <div class="table-responsive" v-if="secilenOrtakSiparisler.length > 0">
						      	<table class="table" v-if="secilenOrtakSiparisler.length > 0">
						      		<thead>
						      			<th scope="col">#</th>
...
var app = new Vue({
  ...
  methods:{
      ...
      var iadeler = this.durumaGoreBul(id, "wc-refunded");
      return Number(hakedis) - Number(iadeler);
  	},
    ...
    toplamSiparisListe(orders){
  	...
  	guncelle(){
  		...
  		const url = this.url + "Home/dashboard/" + data.join("");
    ...
    popupBekleme(){
      ...
      itemList.forEach(t => {
		...
		yuzdeGetir(items, seviye){
			...
			console.log(items.adsoyad, items.seviye)
  ...
  computed: {
    seUser(){
      var index = this.ccustomers.findIndex(c => c.ID == this.user_id)
		...
		customers(){
			const customers = this.treeSelf(this.ccustomers, this.user_id);
			this.seviye(this.tree(this.ccustomers), this.aktifPasif);
			return customers;
		},
